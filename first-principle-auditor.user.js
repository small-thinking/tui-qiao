...
(function() {
    ...
    const BTN_STYLE = `
        ...
        font-size: 13px; font-weight: 600; user-select: none; transition: transform 0.1s;
    `;
        ...
        if (isLoading || !isConfiguring) {
            resultPanel.innerHTML = `
                ...
                <div class="content">
                    ...
                    ${isLoading && !content ? `<div style="text-align:center; padding: 10px;"><span class="loading-spinner"></span> Sifting...</div>` : `<div style="white-space:pre-wrap;">${content}</div>`}
                </div>
            `;
            shadowRoot.getElementById('close-auditor').onclick = () => resultPanel.style.display = 'none';
            shadowRoot.querySelector('.settings-trigger').onclick = () => showConfig();
...
Rules: Absolute Anonymity. Verdict First. Cohesive Synthesis (max 4 sentences). Max 5 sentences total. Provide up to 3 links.`;
        ...
        GM_xmlhttpRequest({
                ...
                lines.forEach(line => {
                    if (line.startsWith("data: ")) {
                        try {
                            ...
                            if (part) {
                                if (part.text) { fullContent += part.text; showResult("Auditing...", fullContent, true, selectedText, fullThought); }
                                if (part.thought) { fullThought += part.thought; showResult("Auditing...", fullContent, true, selectedText, fullThought); }
                            }
                        } catch (e) {}
                    }
                });
            ...
            onload: (response) => {
                if (response.status === 200) showResult("Audit Result", fullContent || "No response.", false, selectedText, fullThought);
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (!floatingBtn) {
            ...
            floatingBtn.style.cssText = BTN_STYLE;
            ...
            floatingBtn.onclick = (ev) => {
                ...
                if (currentSelection) callGemini(currentSelection);
        ...
        floatingBtn.style.display = 'flex'; // 强制显式设为 flex
    });
